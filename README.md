...
5. ✅ **Modifiers** — talismans + physick + buffs + greases. Three effect kinds combine as `AR = (base + flat) × mult`: flat attribute bonuses feed scaling, greases add fixed flat damage, % buffs apply strongest-within-group / multiplicative-across. 45-entry sourced catalogue. Wired into rank & optimize; selectable in the UI. Weapon-buff *spells* (elemental add scales with the catalyst) and spell-scaling buffs remain out of scope.
